...
def titles_union(all_rankings):
    ...
    df = pd.DataFrame(None, index=uniques, columns=uniques)
...
def get_rankings(req, input_string):
    ...
    for key in rankings:
        ...
        for word in add_words:
            ...
            if type(rankings[key]) == str:

                rankings[key] = rankings[key].split(word)

            elif type(rankings[key]) == list:
        ...
        for i in range(len(rankings[key])):
            ...
            for word in ['-inch', 'inch']:
                if word in rankings[key][i].lower():
                    rankings[key][i] = rankings[key][i].replace(word, ' ').replace(word.title(), ' ').replace(
                        word.upper(), ' ')
...
def count_occurrence(sites_content, all_rankings, urls):
    titles_list = get_titles_list(all_rankings)
